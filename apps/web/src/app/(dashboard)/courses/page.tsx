...
async function getCourses(isAdmin: boolean = false, token?: string): Promise<Course[]> {
...
async function getEnrolledCourseIds(token?: string): Promise<Set<string>> {
  if (!token) return new Set();
  // Lấy toàn bộ courses rồi check từng cái — thay thế bằng API batch nếu có
  // Dùng sessions endpoint của student để lấy danh sách đã enroll
  try {
    // Không có batch check API, trả về empty để check trên detail page
    return new Set();
  } catch {
    return new Set();
  }
...
export default async function CoursesPage() {
  ...
  const courses = await getCourses(user.role === "ADMIN", token);

  if (user.role === "ADMIN") {
    return <AdminCoursesPage initialCourses={courses} />;
  }

  // Fetch enrollment status cho tất cả courses song song
  const enrollmentResults = await Promise.all(
    courses.map(async (course) => {
      if (!token) return false;
      try {
        const res = await fetch(`http://localhost:3001/api/v1/enrollments/check/${course.id}`, {
          headers: { Authorization: `Bearer ${token}` },
          cache: "no-store",
        });
  ...
  const enrolledMap = new Map(courses.map((c, i) => [c.id, enrollmentResults[i]]));
  ...
  return (
    <div className="flex flex-col gap-6 animate-fade-in-up">
      ...
      <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
        {courses.map((course) => {
          const isEnrolled = enrolledMap.get(course.id) ?? false;
          return (
            <Link key={course.id} href={`/courses/${course.id}`}>
              <Card className="group h-full flex flex-col hover:border-primary/50 hover:shadow-xl hover:shadow-primary/5 transition-all duration-300 border-slate-200 overflow-hidden glass-effect">
                ...
                <CardFooter className="mt-auto pt-4 border-t border-slate-100 flex justify-between items-center bg-slate-50/50">
                  {isEnrolled ? (
                    <span className="text-sm font-semibold text-emerald-600 group-hover:translate-x-1 transition-transform inline-flex items-center">
